...
@test "codex-bridge: rejects unsupported app-server endpoints" {
  ...
  [ "$status" -eq 1 ]
  [[ "$output" =~ "supports unix://PATH or ws://HOST:PORT" ]]
}
...
# Native Windows uses a ws:// (TCP loopback) app-server because unix-domain
# sockets do not survive the MSYS path / AF_UNIX / Node named-pipe mismatch. The
# WebSocket framing is identical to the unix path; only the connection target
# differs. TCP listen works in every environment, so (unlike the unix test
# above) this leg is not skipped on Windows.
@test "codex-bridge: connects to ws app-server over TCP" {
  run node -e 'const net = require("net"); const crypto = require("crypto"); if (!net || !crypto) process.exit(1);'
  if [ "$status" -ne 0 ]; then
    skip "node net/crypto modules are not available in this sandbox"
  fi
  ...
  local portfile="$TEST_SKILL_DIR/fake-ws-tcp-app-server.port"
...
@test "codex-bridge: refuses when the same identity already has a live bridge" {
  # The bridge proves an existing pidfile is live with process.kill(pid, 0). On
  # Git Bash the shell's $$ is an MSYS pseudo-PID, which the native Windows Node
  # cannot match against a Windows process — it would read as stale, clean the
  # pidfile, and fall through to spawning a default stdio app-server (no
  # --app-server is passed here), which hangs. Production writes a real Windows
  # process.pid, so this guard is test-setup-only.
  case "$(uname -s 2>/dev/null || echo unknown)" in
    MINGW*|MSYS*|CYGWIN*) skip "Git Bash \$\$ is an MSYS PID; not a Windows process for process.kill" ;;
  esac
